...
- Workspace root follows `ctx.sessions` / `ctx.workspaces` and retargets the host via `POST /api/dsh-workbench/workspace`. Switching workspace resets open tabs; review follows the current session without clearing the editor.
