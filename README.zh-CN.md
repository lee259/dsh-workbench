...
- 支持桌面式快捷键：`⌥⌘B`（或会话顶栏 Session log 旁边的「工作台」）展开/收起，`⌘⇧E`（或标签栏的面板按钮）显隐文件树，`⌘P` 打开文件，树搜索只定位不打开，`⌘F` / `⌘L` 在预览里查找或跳行，`⌘W` 关闭当前文件，`⌘1`–`⌘9` 切换标签。树工具栏可一键折叠全部文件夹；也可拖拽或右键把路径插入对话输入框。
...
```bash
dsh plugin --profile web add dsh-workbench@0.3.0
dsh web
```
...
```bash
pnpm dlx @deepseek-ai/dsh plugin --profile web add dsh-workbench@0.3.0
