...
3. The `Publish` workflow publishes `dsh-workbench` to npm and opens a GitHub Release.

GitHub setup:

- Configure [npm trusted publishing](https://docs.npmjs.com/trusted-publishers) for this repository: on npmjs.com, go to `dsh-workbench` → Settings → Trusted Publishing, add `lee259/dsh-workbench` with workflow path `.github/workflows/publish.yml`.
